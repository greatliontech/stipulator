...
func TestPruneScopedWitnessEvaluationAndDeletionOnlyFastPath(t *testing.T) {
	...
	// and executed witness counts (REQ-gap-resolved-pruned): the first
	// run executed the one gap-bound witness and served nothing.
	if !strings.Contains(out, "evaluated 1 gap records: 0 witnesses served, 1 executed") {
		t.Fatalf("prune did not name the evaluation performed:\n%s", out)
	}
	...
	// A verification problem only the witness run's backend exposes —
	// a bound test whose file became generated after the bind — refuses
	// the resolved-record evaluation rather than deleting on a shaky
	// reading; the record stays until the problem is fixed.
	run(0, "gap", "--req", "REQ-pr-a", "--reason", "external judgment", "--manual", "ops signed off")
	run(0, "gap", "--req", "REQ-pr-a", "--fired")
	genTest := "package ok\n\nimport \"testing\"\n\nfunc TestGen(t *testing.T) {\n\tif Double(1) != 2 {\n\t\tt.Fatal(\"gen\")\n\t}\n}\n"
	write("ok/gen_test.go", genTest)
	run(0, "bind", "--req", "REQ-pr-a", "--symbol", "example.com/prunefix/ok.TestGen", "--role", "tests")
	write("ok/gen_test.go", "// Code generated by a fixture. DO NOT EDIT.\n\n"+genTest)
	if out := run(2, "prune"); !strings.Contains(out, "fix verification problems first") || !strings.Contains(out, "generated file") {
		t.Fatalf("prune under a verification problem did not refuse:\n%s", out)
	}
	if _, err := os.Stat(filepath.Join(dir, ".stipulator/gaps/pr-a.textproto")); err != nil {
		t.Fatal("refused prune deleted the gap record")
	}
	run(0, "unbind", "--req", "REQ-pr-a", "--symbol", "example.com/prunefix/ok.TestGen", "--role", "tests")
	if err := os.Remove(filepath.Join(dir, "ok", "gen_test.go")); err != nil {
		t.Fatal(err)
	}
	if out := run(0, "prune"); !strings.Contains(out, "1 resolved gaps pruned") {
		t.Fatalf("repaired tree did not prune:\n%s", out)
	}
