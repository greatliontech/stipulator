...
Since witness tests carry `//gofresh:pure` directives their records publish
and serve across gate runs, so the discarded work shrinks to whatever the
edit actually staled — the double execution remains, but only over the
stale set rather than the whole suite.
