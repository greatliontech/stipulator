...
state, any attestation, each binding with role, the clause it is
scoped to when it is, witness class, and pin freshness, and the seed
symbols its closure binds (its own bindings and its spec
neighborhood's, REQ-context-seeds) — so answering "tell me everything
