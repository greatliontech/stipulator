...
- `backend` (mcp, cli) — language backend (go where absent; a claim's defaults to the call's); on the cli once for all claims or one per claim.
...
**knobs:**
- `backend` (mcp, cli) — backend whose symbols retarget (go where absent; taken once, repetition refused).
